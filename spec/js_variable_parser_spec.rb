...
RSpec.describe JsVariableParser::JsVariableParser do

  context "when 'comma_not\'closed'" do 
    let(:js) { "comma_not'closed" }
    it "raises error" do 
      expect { JsVariableParser::JsVariableParser.new(js).variables }.to raise_error(StandardError)
    end
  end
  ...
  context "when 'moment()'" do 
    ...
    it "detects moment" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["moment"])
    end
  end
  ...
  context "when 'moment().format('YYYY-MM-DD')'" do
    ...
    it "detects .format function" do 
      expect(JsVariableParser::JsVariableParser.new(js).functions).to eq([".format"])
    end
  end
  ...
  context "when 'moment().tz(timezone).unknownFunction().format('YYYY')'" do
    ...
    it "detects moment, timeAone" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["moment", "timeZone"])
    end

    it "detects .format function" do 
      expect(JsVariableParser::JsVariableParser.new(js).functions).to eq([".tz", ".unknownFunction", ".format"])
    end
  end
  ...
  context "when 'moment().format('YYYY-MM-DD') + hoge'" do
    ...
    it "detects [moment, hoge]" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["moment", "hoge"])
    end
  ...
  context "when 'moment().format(formatStr)'" do
    ...
    it "detects [moment, formatStr]" do
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["moment", "formatStr"])
    end
  end
  ...
  context "when 'css + hoge'" do
    ...
    it "detects [css, hoge]" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["css", "hoge"])
    end
  end
  ...
  context "when 'css + 'hoge''" do
    let(:js) { "css + 'hoge'" }
    it "detects [css]" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["css"])
    end
  end
  ...
  context "when 'value === 'hoge''" do
    let(:js) { "value === 'hoge'" }
    it "detects [value]" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["value"])
    end
  end
  ...
  context "when 'hoge[index]" do
    ...
    it "detects [hoge, index]" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq(["hoge", "index"])
    end
  end
  ...
  context "when '`this is a ${penName}`" do 
    ...
    it "cannot detect penName" do 
      expect(JsVariableParser::JsVariableParser.new(js).variables).to eq([])
    end
  end
